...
fjoldihafnir=ddply(gras,.(Ár,harbour),summarize,N=length(names(table(as.factor(id)))),"Magn"=sum(Óslægt)/1000,`Magn/komunr`=round(Magn/as.numeric(N),2))
colnames(fjoldihafnir) <- c("Ár","Höfn","Fjöldi báta","Magn", "Magn/komunr")
#  (hvert komunúmer er löndun)
medallondun <- ddply(fjoldihafnir,.(Ár),summarise,`Meðallöndun (tonn)` = round(mean(`Magn/komunr`),2))
```
...
hc <- medallondun %>% 
  ...
  hc_title(text = "Grásleppa",
           margin = 20, align = "left") %>% 
  hc_subtitle(text = "Magn á hverja löndun",
              align = "left",
              style = list(color = "#2166ac", fontWeight = "bold"))  
...
# Meðalveiði á löndun eftir höfnum
...
hc <- fjoldihafnir %>% 
  hchart(
    'column', hcaes(x = 'Höfn', y = '`Magn/komunr`', group = 'Ár')
    ) %>%
  hc_colors(c('#ca0020','#f4a582','#92c5de','#0571b0')) %>% 
  hc_legend(align = "right", verticalAlign = "top",
            layout = "vertical", x = 0, y = 100) %>% 
